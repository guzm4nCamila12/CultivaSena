...
const Zonas = () => {
  ...
  const getColumnasConPermisos = (isReporte, permisos) => {
    // Columnas base
    const columnasBase = [
      { key: "nombre", label: "Nombre", icon2: zonasIcon },
      { key: "verSensores", label: "Sensores", icon: sensoresIcon, icon2: sensoresIcon, permiso: "ver sensores" },
      { key: "actividades", label: "Actividades", icon: actividadesIcon, icon2: actividadesIcon, permiso: "ver actividades" },
      { key: "acciones", label: "Acciones", icon2: ajustes }
    ];

    // Si es reporte, quitar columnas específicas
    const columnasReportes = isReporte
      ? columnasBase.filter(col => !["verSensores", "acciones"].includes(col.key))
      : columnasBase;

    // Validar permisos
    return columnasReportes.filter(
      col => !col.permiso || permisos[col.permiso]?.tienePermiso
    );
  };

  const columnas = getColumnasConPermisos(isReporte, permisos);

  // Acciones solo si no es reporte
  const acciones = (fila) => (
    <div className="flex justify-center gap-2">
      {fila.nombre !== "Zona general" && (
        permisos["editar zonas"]?.tienePermiso && (
      ...
      {fila.nombre !== "Zona general" && (
        permisos["eliminar zonas"]?.tienePermiso && (
      ...
      {permisos["ver zonas"]?.tienePermiso && (
        <div>
          ...
          <MostrarInfo
            ...
            mostrarAgregar={permisos["crear zonas"]?.tienePermiso && !isReporte}
